<?php
$wikiurl = 'https://star-citizen.wiki/';
...
$systems = scandir('/opt/app/resources/data');
...
if(isset($_POST['refetch']) && $_POST['refetch'] == 1){
    ...
    foreach ($starmap as $key => $value) {
        ...
        if($decode['data']['rowcount'] == 0){
            ...
            $systemdata = fopen("/opt/app/resources/data/".$systemkey.".json", "w");
        	...
        	<div class="row">
        		<div class="col-md-2 sidebar">
                    ...
                    <div class="col-md-12">
                        ...
                        <?php
                        if (file_exists('/opt/app/resources/data/BAKER.json')) {
                            echo '<p style="margin-top: 10px;">Letzte Aktualisierung: '.date ("H:i d.m.Y", filemtime('/opt/app/resources/data/BAKER.json')).'</p>';
                        }
                        ?>
                    </div>
                    <?php
        		...
        		<div class="col-md-10 col-md-offset-2" style="padding-bottom: 20px;">
                    <?php
                    if($system > 0){
                        $data = json_decode(file_get_contents('/opt/app/resources/data/'.$systems[$_POST['system']]), true)['data']['resultset'];
                        $asteroidbelts = '';
                        $jumppoints = '';
                        $planets    = '';
                        $stars      = '';
